...
- To start 'x' service locally using mvn & spring in the root directory. The first service that should be run is config-server then eureka-service, the last service needed to run is the api-gateway:
    ```bash
